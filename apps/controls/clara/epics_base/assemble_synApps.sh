...
then
    ...
    EPICS_BASE=/BUILD_VA/va/epics-base/
...
get_repo()
{
	...
	RELEASE_NAME=$3
	TAG=$4
    if [[ $5 ]]; then URL=$5; else URL='github.com'; fi
	FOLDER_NAME=$MODULE_NAME-${TAG//./-}

	echo
	echo "Grabbing $MODULE_NAME at tag: $TAG $URL $PROJECT $MODULE_NAME"
	echo

	git clone -q --branch $TAG $CLONE_DEPTH https://$URL/$PROJECT/$MODULE_NAME.git $FOLDER_NAME
...
get_support()
{
	git clone -q --branch $2 $CLONE_DEPTH https://github.com/EPICS-synApps/$1.git
}
...
mkdir ${SYNAPPS_DIR}
cp ./assemble_synApps.sh ${SYNAPPS_DIR}
cd ${SYNAPPS_DIR}

get_support support $SUPPORT
mv ./assemble_synApps.sh support
cd support
...
# CSSI modules
STFC="ujo48515:glpat-TaqZcrGgE5RKxMSiY1ek@gitlab.stfc.ac.uk"
GITPATH="CSSI/EPICS/Support"
if [[ $FINS ]];          then   get_repo     $GITPATH               fins           FINS           $FINS           $STFC; fi
if [[ $LAKESHORE224 ]];  then   get_repo     $GITPATH               LakeShore224   LAKESHORE224   $LAKESHORE224   $STFC; fi
if [[ $CRYOCONM14 ]];    then   get_repo     $GITPATH               CryoconM14     CRYOCONM14     $CRYOCONM14     $STFC; fi
if [[ $MIRIONG64 ]];     then   get_repo     $GITPATH               MirionG64      MIRIONG64      $MIRIONG64      $STFC; fi
if [[ $THORTONM800 ]];   then   get_repo     $GITPATH               thorntonM800   THORTONM800    $THORTONM800    $STFC; fi
if [[ $VTF_AUTO_FILL ]]; then   get_repo     $GITPATH               VTF_auto_fill  VTF_AUTO_FILL  $VTF_AUTO_FILL  $STFC; fi
if [[ $STATUS ]];        then   get_repo     $GITPATH               "status"       STATUS         $STATUS         $STFC; fi
if [[ $PHOENIXL300I ]];  then   get_repo     $GITPATH               phoenixL300i   STATUS         $PHOENIXL300I   $STFC; fi
if [[ $SNCSEQ ]];        then   get_repo     $GITPATH               seq   	 	   SNCSEQ         $SNCSEQ	      $STFC; fi
# set empty string for CROSS_COMPILER_TARGET_ARCHS in CROSS_SITE

for files in \
          Galil-3-0-$GALIL optics-$OPTICS love-$LOVE $OPTICS  \
          areaDetector-$AREA_DETECTOR delaygen-$DELAYGEN lua-$LUA  \
          ip-$IP MirionG64-$MIRIONG64 scaler-$SCALER calc-$CALC motor-$MOTOR
do
  sed -i '/CROSS_COMPILER_TARGET_ARCHS/s/.*/CROSS_COMPILER_TARGET_ARCHS =/' ./$files/configure/CONFIG_SITE
done
...
then
	cd asyn-$ASYN
    # EPICS base is commented out
	sed -i s:'#EPICS_BASE':'EPICS_BASE':g configure/RELEASE
