...
class BeamToScreen(object):
    def __init__(self, screenDict, beam, verbose=False):
        """
        ...
        general_tools.check_dict_params(
            screenDict, self.required_keys, "screen data", "beamToScreen"
        )
        ...
        # find overlaps
        self.xi_overlaps, self.yi_overlaps = self.get_pix_overlap_inds(
            xx_big=self.xx_vals,
            yy_big=self.yy_vals,
            xx_small=self.beam_xx,
            yy_small=self.beam_yy,
        )
        ...
        self.pix_vals_raw = self.add_beam_to_background(
            sorted_index_x=self.xi_overlaps,
            sorted_index_y=self.yi_overlaps,
            z_small=self.beam_processed_pix,
            z_big=self.bckgrnd,
        )
    ...
    def set_background(self):
        self.bckgrnd = np.full(self.xx_vals.shape, self.base_constant, dtype="float64")
    ...
    def set_beam_grid(self):
        ...
        ind_x_min, ind_x_max, ind_y_min, ind_y_max = self.correct_indices(
        ...
        self.beam_grid_coords = np.append(
            self.beam_xx.reshape(-1, 1), self.beam_yy.reshape(-1, 1), axis=1
        )
    ...
    def evaluate_beam_KDE(self):
        if self.KDE_method == "scipy_gaussian":
            ...
            self.beam_pix_vals_pdf = beam_pix_vals.reshape(
                len(self.beam_y_vals), len(self.beam_x_vals)
            )
    ...
    def add_beam_to_background(self, sorted_index_x, sorted_index_y, z_big, z_small):
        ...
        z_big[
            sorted_index_y[:, None], sorted_index_x
        ] += z_small  # the index in big corresponds to the value at the index in small
    ...
    def apply_physical_res_limit(self):
        self.beam_processed_pix = gaussian_filter(
            self.beam_pix_vals_pdf, sigma=[self.pix_res_x, self.pix_res_y]
        )

    def post_process_image_pix(self, verbose=False):
        """
    ...
    def set_digitise_pix_by_max_value(self, bit_depth, max_pix_val, verbose=False):
        """
        ...
        if max_pix_val > max_pix:
            raise Exception(
                "Maximum pixel value cannot be higher than allowed by bit depth"
            )
    ...
    def set_digitise_pix_by_charge_dens(
        self, bit_depth, saturation_charge_dens, verbose=False
    ):
        """
