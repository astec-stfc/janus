...
def control_file_overwrite_logic(filename, overwrite_existing):
    ...
    if not overwrite_existing:
        if os.path.isfile(filename):
            print(
                "File found, overwriting not selected, sending flag to stop file write process"
            )
            continue_flag = False
        else:
        ...
        if os.path.isfile(filename):
            print(
                "File found, overwriting is selected, sending flag to continue file write process"
            )
            continue_flag = True
...
def gradient_gauss_blur(image, setup_dict):
    ...
    check_dict_params(
        setup_dict,
        required_vals,
        dict_name="image blur dict",
        output_type="gradient blur",
    )
    ...
    for i, slice in enumerate(imageslices):
        if split_axis == 0:
            imageslices[i] = gaussian_filter(
                slice, sigma=[SIG_VAL[i], base_blur], mode="nearest"
            )
            reformedimage = np.concatenate(imageslices, axis=split_axis)
            processedimage = uniform_filter(
                reformedimage, size=[process_blur, 0], mode="constant"
            )

        elif split_axis == 1:
            imageslices[i] = gaussian_filter(
                slice, sigma=[base_blur, SIG_VAL[i]], mode="nearest"
            )
            reformedimage = np.concatenate(imageslices, axis=split_axis)
            processedimage = uniform_filter(
                reformedimage, size=[0, process_blur], mode="constant"
            )
